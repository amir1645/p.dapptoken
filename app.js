...
// تابع بروزرسانی اطلاعات توکن P - نسخه اصلاح شده
async function updatePTokenInfo() {
    ...
    try {
        ...
        console.log('Initial price:', initialPrice, 'Growth:', growthPercentage);
        
        // به‌روزرسانی UI
        document.getElementById('p-token-price').textContent = currentPriceNum.toFixed(8);
        ...
        document.getElementById('initial-price').textContent = `${initialPriceNum.toFixed(8)} پالیگان`;
...
// تابع نمایش ژنولوژی - نسخه کاملاً اصلاح شده
async function displayGenealogyTree() {
    ...
    try {
        ...
        const treeHTML = await buildGenealogyTree(user.id);
...
// تابع جدید برای ساخت درخت ژنولوژی
async function buildGenealogyTree(userId) {
    try {
        let html = '<div class="tree-levels">';
        
        // سطح 0 - کاربر اصلی
        const mainUser = await contract._getSpecialUserInfoForMigrateToNewFork(userId);
        const isCurrentUser = mainUser.userAddress.toLowerCase() === userAccount.toLowerCase();
        
        html += `
            <div class="tree-level level-0">
                <div class="tree-node">
                    <div class="node-connector main"></div>
                    <div class="node-box ${isCurrentUser ? 'current-user' : ''}">
                        <div class="node-id">${mainUser.id.toString()}</div>
                        <div class="node-stats">
                            <div class="node-stat left">
                                <i class="fas fa-arrow-left"></i>
                                <div class="node-stat-count">${mainUser.leftCount.toString()}</div>
                            </div>
                            <div class="node-stat right">
                                <i class="fas fa-arrow-right"></i>
                                <div class="node-stat-count">${mainUser.rightCount.toString()}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            if (directs.leftId.toString() !== '0') {
                ...
                html += `
                    <div class="tree-node">
                        ...
                        <div class="node-box">
                            <div class="node-id">${leftUser.id.toString()}</div>
                            <div class="node-stats">
                                <div class="node-stat left">
                                    <i class="fas fa-arrow-left"></i>
                                    <div class="node-stat-count">${leftUser.leftCount.toString()}</div>
                                </div>
                                <div class="node-stat right">
                                    <i class="fas fa-arrow-right"></i>
                                    <div class="node-stat-count">${leftUser.rightCount.toString()}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            if (directs.rightId.toString() !== '0') {
                ...
                html += `
                    <div class="tree-node">
                        ...
                        <div class="node-box">
                            <div class="node-id">${rightUser.id.toString()}</div>
                            <div class="node-stats">
                                <div class="node-stat left">
                                    <i class="fas fa-arrow-left"></i>
                                    <div class="node-stat-count">${rightUser.leftCount.toString()}</div>
                                </div>
                                <div class="node-stat right">
                                    <i class="fas fa-arrow-right"></i>
                                    <div class="node-stat-count">${rightUser.rightCount.toString()}</div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        // سطح 2 - زیرمجموعه‌های سطح دوم (اختیاری)
        html += await buildSecondLevel(directs);
        
        html += '</div>';
        return html;
        
    } catch (err) {
        console.error('Error building genealogy tree:', err);
        return `
            <div class="tree-placeholder">
                <div class="placeholder-icon">
                    <i class="fas fa-exclamation-triangle"></i>
                </div>
                <p>خطا در ساخت درخت ژنولوژی</p>
                <span>${err.message}</span>
            </div>
        `;
    }
...
// تابع برای ساخت سطح دوم درخت
async function buildSecondLevel(directs) {
    let html = '';
    let hasSecondLevel = false;
    
    // بررسی زیرمجموعه‌های سطح دوم برای چپ
    if (directs.leftId.toString() !== '0') {
        const leftDirects = await contract.getUserDirects(directs.leftId);
        if (leftDirects.leftId.toString() !== '0' || leftDirects.rightId.toString() !== '0') {
            hasSecondLevel = true;
        }
    }
    
    // بررسی زیرمجموعه‌های سطح دوم برای راست
    if (directs.rightId.toString() !== '0') {
        const rightDirects = await contract.getUserDirects(directs.rightId);
        if (rightDirects.leftId.toString() !== '0' || rightDirects.rightId.toString() !== '0') {
            hasSecondLevel = true;
        }
    }
    ...
    if (hasSecondLevel) {
        ...
        // زیرمجموعه‌های سطح دوم برای چپ
        if (directs.leftId.toString() !== '0') {
            const leftDirects = await contract.getUserDirects(directs.leftId);
            
            // چپ-چپ
            if (leftDirects.leftId.toString() !== '0') {
                const leftLeftUser = await contract._getSpecialUserInfoForMigrateToNewFork(leftDirects.leftId);
                html += `
                    <div class="tree-node">
                        <div class="node-connector left"></div>
                        <div class="node-box small">
                            <div class="node-id">${leftLeftUser.id.toString()}</div>
                        </div>
                    </div>
                `;
            } else {
                html += '<div class="tree-node"><div class="empty-node small"></div></div>';
            }
            
            // چپ-راست
            if (leftDirects.rightId.toString() !== '0') {
                const leftRightUser = await contract._getSpecialUserInfoForMigrateToNewFork(leftDirects.rightId);
                html += `
                    <div class="tree-node">
                        <div class="node-connector right"></div>
                        <div class="node-box small">
                            <div class="node-id">${leftRightUser.id.toString()}</div>
                        </div>
                    </div>
                `;
            } else {
        ...
        // زیرمجموعه‌های سطح دوم برای راست
        if (directs.rightId.toString() !== '0') {
            const rightDirects = await contract.getUserDirects(directs.rightId);
            
            // راست-چپ
            if (rightDirects.leftId.toString() !== '0') {
                const rightLeftUser = await contract._getSpecialUserInfoForMigrateToNewFork(rightDirects.leftId);
                html += `
                    <div class="tree-node">
                        <div class="node-connector left"></div>
                        <div class="node-box small">
                            <div class="node-id">${rightLeftUser.id.toString()}</div>
                        </div>
                    </div>
                `;
            } else {
            ...
            // راست-راست
            if (rightDirects.rightId.toString() !== '0') {
                const rightRightUser = await contract._getSpecialUserInfoForMigrateToNewFork(rightDirects.rightId);
                html += `
                    <div class="tree-node">
                        <div class="node-connector right"></div>
                        <div class="node-box small">
                            <div class="node-id">${rightRightUser.id.toString()}</div>
                        </div>
                    </div>
                `;
            } else {
...
async function refreshGenealogy() {
    ...
    try {
        ...
        showMessage('خطا در به‌روزرسانی ژنولوژی', 'error');
    }
}
