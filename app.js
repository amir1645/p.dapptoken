...
// تنظیمات کارمزد
const FEE_PERCENTAGE = 1; // 1% کارمزد
...
function disconnectWallet() {
    ...
    // ریست کردن مقادیر
    document.getElementById('p-token-price').textContent = '0.00001000';
    document.getElementById('p-token-balance').textContent = '0.000000';
    document.getElementById('p-token-value').textContent = '≈ 0 پالیگان';
    
    showMessage('اتصال قطع شد', 'info');
}

// تابع محاسبه کارمزد ثبت‌نام
function calculateRegistrationFee() {
    const basePrice = 350;
    const fee = (basePrice * FEE_PERCENTAGE) / 100;
    const totalPrice = basePrice + fee;
    
    document.getElementById('final-registration-price').textContent = totalPrice.toFixed(1) + ' پالیگان';
}
...
async function register() {
    ...
    try {
        ...
        const basePrice = ethers.utils.parseEther('350');
        const fee = basePrice.mul(FEE_PERCENTAGE).div(100);
        const totalPrice = basePrice.add(fee);
...
async function updatePTokenInfo() {
    ...
    try {
        ...
        const tokenValue = parseFloat(formattedBalance) * priceInMatic;
        
        // قیمت اولیه دقیق - 0.00001 پالیگان
        const initialPrice = 0.00001;
        
        // محاسبه درصد رشد نسبت به قیمت اولیه
        const growthPercentage = ((priceInMatic - initialPrice) / initialPrice) * 100;
        
        // به‌روزرسانی UI با قیمت واقعی و دقت بالا
        document.getElementById('p-token-price').textContent = priceInMatic.toFixed(8);
        document.getElementById('p-token-balance').textContent = parseFloat(formattedBalance).toFixed(6);
        document.getElementById('p-token-value').textContent = `≈ ${tokenValue.toFixed(6)} پالیگان`;
        document.getElementById('growth-percentage').textContent = `${growthPercentage.toFixed(4)}%`;
        document.getElementById('initial-price').textContent = `${initialPrice.toFixed(8)} پالیگان`;
        ...
        console.log('Token Price:', priceInMatic, 'Balance:', formattedBalance, 'Growth:', growthPercentage + '%');
        
    } catch (err) {
        console.error('Error updating P token info:', err);
        // نمایش اطلاعات نمونه در صورت خطا
        const initialPrice = 0.00001;
        document.getElementById('p-token-price').textContent = initialPrice.toFixed(8);
        document.getElementById('p-token-balance').textContent = '0.000000';
        document.getElementById('p-token-value').textContent = '≈ 0 پالیگان';
...
async function calculateBuyTokens() {
    ...
    try {
        ...
        // محاسبات کارمزد
        const fee = (buyAmount * FEE_PERCENTAGE) / 100;
        const totalAmount = buyAmount + fee;
        
        // محاسبه تعداد توکن دریافتی
        const tokensReceived = buyAmount / priceInMatic;
        
        // نمایش نتایج با دقت بالا
        document.getElementById('buy-amount-display').textContent = buyAmount.toFixed(6) + ' پالیگان';
        document.getElementById('buy-fee').textContent = fee.toFixed(6) + ' پالیگان';
        document.getElementById('buy-total').textContent = totalAmount.toFixed(6) + ' پالیگان';
        document.getElementById('tokens-received').textContent = tokensReceived.toFixed(8) + ' توکن P';
        document.getElementById('current-token-price').textContent = priceInMatic.toFixed(8) + ' پالیگان';
...
async function calculateSellMatic() {
    ...
    try {
        ...
        // محاسبه مبلغ فروش
        const sellValue = sellAmount * priceInMatic;
        
        // محاسبات کارمزد
        const fee = (sellValue * FEE_PERCENTAGE) / 100;
        const totalReceived = sellValue - fee;
        
        // نمایش نتایج با دقت بالا
        document.getElementById('sell-amount-display').textContent = sellValue.toFixed(6) + ' پالیگان';
        document.getElementById('sell-fee').textContent = fee.toFixed(6) + ' پالیگان';
        document.getElementById('sell-total').textContent = totalReceived.toFixed(6) + ' پالیگان';
        document.getElementById('matic-received').textContent = totalReceived.toFixed(8) + ' پالیگان';
        document.getElementById('current-sell-price').textContent = priceInMatic.toFixed(8) + ' پالیگان';
...
// تابع ریست محاسبات خرید
function resetBuyCalculations() {
    document.getElementById('buy-amount-display').textContent = '0 پالیگان';
    document.getElementById('buy-fee').textContent = '0 پالیگان';
    document.getElementById('buy-total').textContent = '0 پالیگان';
    document.getElementById('tokens-received').textContent = '0.000000 توکن P';
}

// تابع ریست محاسبات فروش
function resetSellCalculations() {
    document.getElementById('sell-amount-display').textContent = '0 پالیگان';
    document.getElementById('sell-fee').textContent = '0 پالیگان';
    document.getElementById('sell-total').textContent = '0 پالیگان';
    document.getElementById('matic-received').textContent = '0.000000 پالیگان';
}
...
async function buyPTokens() {
    ...
    try {
        ...
        // محاسبه کل مبلغ با کارمزد
        const fee = (parseFloat(buyAmount) * FEE_PERCENTAGE) / 100;
        const totalAmount = parseFloat(buyAmount) + fee;
        
        if (balanceInMatic < totalAmount) {
            showMessage(`موجودی پالیگان کافی نیست. موجودی شما: ${balanceInMatic.toFixed(4)} MATIC - مورد نیاز: ${totalAmount.toFixed(4)} MATIC`, 'error');
            return;
        }
...
async function renderTreeLevel(nodes, level) {
    ...
    for (const node of nodes) {
        ...
        html += `<div class="node-content ${nodeClass}" onclick="showUserInfo('${node.id}')">`;
...
async function showUserInfo(userId) {
    try {
        ...
        document.getElementById('panel-upline-id').textContent = userInfo.uplineId.toString();
        document.getElementById('panel-left-count').textContent = userInfo.leftCount.toString();
        document.getElementById('panel-right-count').textContent = userInfo.rightCount.toString();
        document.getElementById('panel-position').textContent = getPositionText(userId === currentRootId ? 'root' : 'user');
        
        document.getElementById('user-info-panel').style.display = 'block';
        
    } catch (err) {
        console.error('Error showing user info:', err);
        showMessage('خطا در دریافت اطلاعات کاربر', 'error');
    }
}

// تابع بستن پنل اطلاعات کاربر
...
function applyZoom() {
    ...
    const advancedTree = document.querySelector('.advanced-tree');
    const zoomLevel = document.getElementById('zoom-level');
    
    if (treeViewport && advancedTree) {
        advancedTree.style.transform = `scale(${currentZoom})`;
...
// تابع بازگشت به درخت خود کاربر
async function navigateToMyTree() {
    if (!contract || !userAccount) return;
    ...
    try {
        ...
        currentRootId = user.id.toString();
        await displayGenealogyTree();
        showMessage('بازگشت به درخت شما', 'success');
    } catch (err) {
        console.error('Navigate to my tree error:', err);
    }
}
